...
const analyzeTranscriptWithLLM = async (text: string, question: string): Promise<SpeechAnalysis> => {
  ...
  const basicIsComplete = wordCount >= 5 && !hasActiveFiller && (hasPunctuation || wordCount > 15)
...
export function useVoiceAgent({
  ...
  const llmCheckThrottleMs = 1500 // Reduced from 3000ms to 1500ms for faster response detection
    ...
    recognition.onresult = async (event: any) => {
      ...
      if (currentText) {
        ...
        const shouldCheckLLM =
          now - lastLLMCheckRef.current > llmCheckThrottleMs && currentText.split(/\s+/).length >= 10
        ...
        if (shouldCheckLLM) {
          ...
          if (analysis.llmIsComplete && analysis.llmConfidence && analysis.llmConfidence > 0.85) {
        ...
        }, 2500) // Reduced from 3000ms to 2500ms for faster turn completion
      }
    }
  ...
  }, [audioThreshold, enableBargeIn, isSpeechDetected])
