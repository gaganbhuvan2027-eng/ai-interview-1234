...
export function ScheduledInterviewsSection() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const params = new URLSearchParams({
          ...
          duration: '30' // Default duration for scheduled interviews
        })
  ...
  return (
    <Card className="border-0 shadow-sm mb-12" id="scheduled">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Calendar className="w-6 h-6 text-blue-600" />
          Scheduled Interviews
        </CardTitle>
        <CardDescription>Your upcoming interview sessions scheduled by your institution</CardDescription>
      </CardHeader>
      <CardContent>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <h3 className="font-semibold text-gray-900 text-lg">{interview.course}</h3>
                      <Badge
                        className={
                    ...
                    <p className="text-sm text-gray-600 mb-1">{interview.institution?.name}</p>
                    {interview.batch && (
                      <p className="text-xs text-gray-500 mb-3">Batch: {interview.batch.name}</p>
                    )}
                    ...
                    {interview.deadline && (
                      <div className="mt-2 flex items-center gap-2">
                        <AlertCircle className={`w-4 h-4 ${expired ? 'text-red-600' : 'text-orange-600'}`} />
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    {!isToday && !isWithin24Hours && !isPast && !expired && (
                      <Badge variant="outline" className="text-gray-600">
                        Starts in{" "}
                        {Math.ceil((scheduleDate.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24))} days
                      </Badge>
                    )}
                    {(isPast || expired) && (
